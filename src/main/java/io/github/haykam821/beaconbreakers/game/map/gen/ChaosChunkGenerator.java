...
public final class ChaosChunkGenerator extends ChunkGenerator {
    public static final Codec<ChaosChunkGenerator> CODEC = RecordCodecBuilder.create((instance) -> {
    ...
    private static final float[] BIOME_WEIGHT_TABLE = (float[])Util.make(new float[25], (fs) -> {
        for(int i = -2; i <= 2; ++i) {
            for(int j = -2; j <= 2; ++j) {
    ...
    private ChaosChunkGenerator(BiomeSource biomeSource, BiomeSource biomeSource2, long worldSeed, Supplier<ChunkGeneratorSettings> supplier) {
        super(biomeSource, biomeSource2, ((ChunkGeneratorSettings)supplier.get()).getStructuresConfig(), worldSeed);
    ...
    public ChunkGenerator withSeed(long seed) {
        return new ChaosChunkGenerator(this.biomeSource.withSeed(seed), seed, this.settings);
    }
